...
module Squib
  class Card
    ...
    def preprocess_save?(batch)
      batch.rotate != false || batch.trim > 0 || !(batch.shadow_radius.nil?)
    end
    ...
    def drop_shadow(cc, s, batch)
      ...
      blur(new_cc, s_rad) do
        ...
        new_cc.rounded_rectangle(s_rad + off_x, s_rad + off_y,
                                cc.target.width, cc.target.height,
                                batch.trim_radius, batch.trim_radius)
        new_cc.fill
      end
